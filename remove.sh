#!/usr/bin/env bash
# remove.sh — Discover all modules and run their remove.sh if present
set -euo pipefail
...
for module_dir in "$SCRIPT_DIR"/*/; do
    ...
    [[ "$module" != .* ]] || continue
    script="$module_dir/remove.sh"
    [[ -f "$script" ]] || continue

    info "Removing module: $module"
    if bash "$script"; then
        info "$module — done"
    else
...
if (( ${#failed[@]} )); then
    ...
    info "All modules removed successfully."
fi

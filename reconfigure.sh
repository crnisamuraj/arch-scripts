#!/usr/bin/env bash
# reconfigure.sh — Discover all modules and run their reconfigure.sh if present
set -euo pipefail
...
for module_dir in "$SCRIPT_DIR"/*/; do
    ...
    [[ "$module" != .* ]] || continue
    script="$module_dir/reconfigure.sh"
    [[ -f "$script" ]] || continue

    info "Reconfiguring module: $module"
    if bash "$script"; then
        info "$module — done"
    else
...
if (( ${#failed[@]} )); then
    ...
    info "All modules reconfigured successfully."
fi
